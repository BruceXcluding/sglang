...
def ck_shape_test(Fp8: bool):
    for n,k in [(1536,7168), (3072,1536), (576,7168), (7168, 256), (7168, 2048), (4608, 7168), (7168, 2304), (512, 7168), (4096, 512)]:
        for m in [1,2,4,8,16,24,32,48,64,96,128,256,512,1024,1536,2048,4096,8192,16384,20480]:
            if Fp8:
    ...
    for m in [1,2,4,8,16,24,32,48,64,96,128,256,512,1024,1536,2048,4096,8192,16384,20480]:
        for n, k in [(1536,7168), (3072,1536), (576,7168), (7168, 256), (7168, 2048), (4608, 7168), (7168, 2304), (512, 7168), (4096, 512)]:
            if Fp8:
                ...
                ck_res = torch.zeros([a_8.shape[0], b_8.shape[0]], dtype=torch.half, device=a.device)
                ...
                torch_rowwise = torch.mul(torch.mul(torch_res, alpha_row.to(torch.float)), alpha_col.to(torch.float)).to(torch.half)
            # INT8
            else:
                ...
                ck_res = torch.zeros([a.shape[0], b.shape[0]], dtype=torch.half, device=a.device)
            ...
            if not torch.allclose(torch_rowwise, ck_res, 1e-4, 1e-4, True):
                ...
                print(f"m: {m}, n: {n}, k: {k}, # not close: {idx[0].shape[0]}, "
